...
画面間は黒フェード（0.25秒×2、`_transition()` + `FadeRect`）を挟んで切り替える。
起動時はタイトルへフェードインで入る。ヘッドレス実行時はフェードなしで即時切替（テスト用）。
...
- 選択キャラの `game_background` を中央640×720（天地いっぱい・アスペクト維持で左右クロップの cover 表示）
  に表示し、表情差分素材がある場合は同じ枠で差し替える。素材は顔と胸のバストアップ正方形画像を想定
  （詳細は `asset_list.md`）。
- ブラシ（`brush.gd`）:
